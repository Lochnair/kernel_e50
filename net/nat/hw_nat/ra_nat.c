...
#include "../../../drivers/net/ethernet/mediatek/gsw_mt7620.h"
#include "../../../drivers/net/ethernet/mediatek/mtk_eth_soc.h"
#include <linux/gfp.h>
...
extern struct FoeEntry *PpeFoeBase_indrv;
extern dma_addr_t PpePhyFoeBase_indrv;
extern struct mutex mdio_nest_lock;

/* After back-ported MTK hwnat. 
 * We expanded the ag_map array to 7x7 because we are counting port number via vlan id (4088 - 4094).
 * So we can handle rx/tx counting of 6 router ports (eth0 - eth5) and switch port (switch0).
 *
 * Notice - The taffic counting of switched ports is not here but we need the ag_map index to count
 *			tx/rx of the router port which related to the switch port.
 */
#define PORT_NUM 6
#define AG_INPUT_NUM (PORT_NUM + 1)
...
static int FoeAllocTbl(uint32_t NumOfEntry)
{
	...
	if (PpeFoeBase == NULL) {
		while (1) {
			...
			if (PpeFoeBase)
				break;
			if (time_after(jiffies, t_start + HZ * 100))
				break;
			usleep_range(10, 20);
		}
	}
...
int32_t PpeParseLayerInfo(struct sk_buff * skb)
{
	...
	PpeParseResult.eth_type = skb->vlan_proto;
	...
	if (is8021Q(PpeParseResult.eth_type) || isSpecialTag(PpeParseResult.eth_type) || isHwVlanTx(skb)) {
...
#else
		PpeParseResult.vlan1_gap = 0; //VLAN_HLEN;
		PpeParseResult.vlan_layer++;
		vh = (struct vlan_hdr *)(skb->data + ETH_HLEN);
#endif
		PpeParseResult.vlan1 = htons(skb_vlan_tag_get_id(skb));

		/* VLAN + PPPoE */
		if (ntohs(eth->h_proto) == ETH_P_PPP_SES) {
		...
		} else if ( is8021Q(eth->h_proto) || 
			isSpecialTag(eth->h_proto)) {
			...
			/* VLAN + IP */
			PpeParseResult.eth_type = eth->h_proto; //vh->h_vlan_encapsulated_proto;
		}
	...
	skb_set_network_header(skb, ETH_HLEN + PpeParseResult.vlan1_gap + PpeParseResult.vlan2_gap + PpeParseResult.pppoe_gap);
	...
	if ((PpeParseResult.eth_type == htons(ETH_P_IP)) || (PpeParseResult.eth_type == htons(ETH_P_PPP_SES)
		...
		if (iph->protocol == IPPROTO_TCP) {
			...
			skb_set_transport_header(skb, ETH_HLEN + PpeParseResult.vlan1_gap + PpeParseResult.vlan2_gap + PpeParseResult.pppoe_gap + (iph->ihl * 4));
			...
			//LAYER4_HEADER(skb) = ((uint8_t *) iph + iph->ihl * 4);
			skb_set_transport_header(skb, ETH_HLEN + PpeParseResult.vlan2_gap + PpeParseResult.pppoe_gap + (iph->ihl * 4));
		...
		if (ip6h->nexthdr == NEXTHDR_TCP) {
			//LAYER4_HEADER(skb) = ((uint8_t *) ip6h + sizeof(struct ipv6hdr));
			skb_set_transport_header(skb, ETH_HLEN + PpeParseResult.vlan2_gap + 
										PpeParseResult.pppoe_gap + sizeof(struct ipv6hdr));
			//th = (struct tcphdr *)LAYER4_HEADER(skb);
			th = (struct tcphdr *)skb_transport_header(skb);
			memcpy(&PpeParseResult.th, th, sizeof(struct tcphdr));
...
int32_t PpeFillInL4Info(struct sk_buff * skb, struct FoeEntry * foe_entry)
{
	...
	if (PpeParseResult.pkt_type == IPV4_HNAPT) {
		...
		if (PpeParseResult.iph.protocol == IPPROTO_TCP) {
			 ...
			 * so we disable HNAT acceleration for this UDP flow
			 */
...
/*PPTP/L2TP LAN->WAN  bind to CPU*/
	...
	return 1;
	    ...
	    if(IS_IPV4_GRP(foe_entry)) {
#if 1
		uint16_t output = (foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) - PORT_VID_BASE(ubnt_bd_g.type);
		uint16_t input = FOE_SP(skb);
		if (input < AG_INPUT_NUM && output < AG_OUTPUT_NUM) {
			PpeSetInfoBlk2(&foe_entry->ipv4_hnapt.iblk2, 1, 0x3F, (uint32_t)ag_map[input][output]);
		} else {
			PpeSetInfoBlk2(&foe_entry->ipv4_hnapt.iblk2, 1, 0x3F, 0);
		}
#else
#if 1//defined (CONFIG_RAETH_SPECIAL_TAG)
...
#if 1
		uint16_t output = (foe_entry->ipv6_5t_route.vlan1 & VLAN_VID_MASK) - PORT_VID_BASE(ubnt_bd_g.type);
		uint16_t input = FOE_SP(skb);
		if (input < AG_INPUT_NUM && output < AG_OUTPUT_NUM) {
			PpeSetInfoBlk2(&foe_entry->ipv6_5t_route.iblk2, 1, 0x3F, (uint32_t)ag_map[input][output]);
		} else {
			PpeSetInfoBlk2(&foe_entry->ipv6_5t_route.iblk2, 1, 0x3F, 0);
		}
#else
#if 1//defined (CONFIG_RAETH_SPECIAL_TAG)
#ifdef CONFIG_WAN_AT_P4
...
static int foe_table_show_entry(struct FoeEntry *entry, u32 hash)
{
...
int32_t PpeTxHandler(struct sk_buff *skb, int gmac_no)
{
     ...
     */
	if (likely((skb->cvm_reserved & SKB_CVM_RESERVED_MASK)))
		FOE_ALG(skb) = 1;
...
/*WAN->LAN accelerate*/
	...
	if (PpeParseLayerInfo(skb)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeParseLayerInfo failed !!!!!", __func__);
		}
	    return 1;
	}
	...
	if (PpeFillInL2Info(skb, foe_entry)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeFillInL2Info (%d) failed !!!!!", __func__, FOE_ENTRY_NUM(skb));
		}
	    return 1;
	}
	...
	if (PpeFillInL3Info(skb, foe_entry)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeFillInL3Info failed !!!!!", __func__);
		}
	    return 1;
	}
	...
	if (PpeFillInL4Info(skb, foe_entry)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeFillInL4Info failed !!!!!", __func__);
		}
	    return 1;
	}
	...
	if (PpeSetForcePortInfo(skb, foe_entry, gmac_no)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeSetForcePortInfo failed !!!!!", __func__);
		}
	    return 1;
	}
	...
	if (PpeSetExtIfNum(skb, foe_entry)) {
	    memset(FOE_INFO_START_ADDR(skb), 0, FOE_INFO_LEN);
		if (DebugLevel >= 3) {
			printk(KERN_ERR "%s: PpeSetExtIfNum failed !!!!!", __func__);
		}
	    return 1;
	}
	...
	if (DebugLevel >= 3) {
		foe_table_show_entry(foe_entry, FOE_ENTRY_NUM(skb));
	}
...
static void PpeSetDstPort(uint32_t Ebl)
{
	...
	DstPort[DP_GMAC] = ra_dev_get_by_name("itf0");//ra_dev_get_by_name("eth2");
#ifdef CONFIG_RAETH_GMAC2
...
static ssize_t hwnat_proc_write(struct file *file,const char __user *buffer,size_t count,loff_t *pos)
{
	char mode;
	...
	if (count > 0) {
		...
		if (mode != '1') {
			// Set address frame forward to CPU
			SetGdmaFwd(0);
			ra_sw_nat_hook_rx = NULL;
			ra_sw_nat_hook_tx = NULL;
			
			// Notice - Don't set PPE_FOE_BASE to 0 if this module is built-in
			// We just need to erase the memory of FOE table
			memset(PpeFoeBase, 0, FOE_4TB_SIZ * sizeof(struct FoeEntry));
		} else {			
...
static int hwnat_proc_show(struct seq_file *seq,void *v)
{
	seq_puts(seq, (ra_sw_nat_hook_rx && ra_sw_nat_hook_tx) ? "1\n" : "0\n");
	return 0;
}
 
static int hwnat_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, hwnat_proc_show, inode->i_private);
}

static const struct file_operations hwnat_proc_fops = {
	.owner   = THIS_MODULE,
	.open    = hwnat_proc_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.write   = hwnat_proc_write,
...
{
	uint32_t data = 0;
	struct proc_dir_entry *mt7621_dir = NULL;
...
#ifdef CONFIG_DTB_UBNT_ER
	// In E50 series, hwnat module is built-in, so please enable hook via proc setting
	SetGdmaFwd(0);
	ra_sw_nat_hook_tx = NULL;
    ra_sw_nat_hook_rx = NULL;
#else
...
/*For FastPath disable crc check*/
	...
	mt7621_dir = proc_mkdir("mt7621", NULL);
	proc_create("hw_nat", 0644, mt7621_dir, &hwnat_proc_fops);

	return 0;
}

static int mtk_ppe_busy_wait(void)
{
	unsigned long t_start = jiffies;
	u32 r = 0;
