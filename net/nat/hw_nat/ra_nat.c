...
#if 1
#if defined (CONFIG_RAETH_SPECIAL_TAG)
		uint16_t output = (foe_entry->ipv4_hnapt.vlan1 & VLAN_VID_MASK) - PORT_VID_BASE;
...
#if 1
#if defined (CONFIG_RAETH_SPECIAL_TAG)
		uint16_t output = (foe_entry->ipv6_5t_route.vlan1 & VLAN_VID_MASK) - PORT_VID_BASE;
		uint16_t input = FOE_SP(skb);
		if (input < AG_INPUT_NUM && output < AG_OUTPUT_NUM) {
...
void update_foe_ac_timer_handler(unsigned long unused)
{
	...
	for (i = 0; i < PORT_NUM; i++) {
		...
		extern int (*is_switch0_member)(u32 port);
		if (is_switch0_member != NULL && is_switch0_member(i))
			continue;
