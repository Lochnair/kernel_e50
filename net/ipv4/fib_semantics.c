...
// Create this function for get correct gateway IP
int ip_fib_find_gateway(__be32 *gw, struct net_device *dev)
{
	struct hlist_head *head;
	struct fib_nh *nh;
	unsigned int hash;
	...
	head = &fib_info_devhash[hash];
	hlist_for_each_entry(nh, head, nh_hash) {
		if (nh->nh_dev == dev && nh->nh_gw != 0 && !(nh->nh_flags & RTNH_F_DEAD)) {
			//printk(KERN_ERR "%s: dev->name = %s, gw = %pI4, nh_flags = %d\n", __func__, nh->nh_dev->name, &nh->nh_gw, nh->nh_flags);
			*gw = nh->nh_gw;
			spin_unlock(&fib_info_lock);
			return 0;
		}
...
EXPORT_SYMBOL(ip_fib_find_gateway);
