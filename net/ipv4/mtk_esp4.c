...
extern int xfrm_rcv_cb(struct sk_buff *skb, unsigned int family, u8 protocol, int err);
extern struct dst_entry *skb_dst_pop(struct sk_buff *skb);
...
)
{
	...
	errVal = ipsec_cmdHandler_cmdDescp_set(currAdapterPtr, cmdHandlerDir, cipherAlg, hashAlg, crypto_aead_authsize(aead)/sizeof(unsigned int), cipherMode, enHmac, aesKeyLen, cipherKey, keyLen, x->id.spi, padCrtlStat);
	if (errVal < 0)
	{
		goto free_addrsPreComputes;
	}
