...
//UBNT - Check model then overlay device tree, before partition table created
void get_model(char *model)
{
	...
	if ((argptr = strstr(argptr, "model=")) != NULL)
	{
		argptr += strlen("model=");
		strncpy(model, argptr, 8);
	}
...
static int read_ubnt_bd(struct ubnt_bd_t *ubnt_bd)
{
	...
	printk("Read UBNT BD OK!\n");
	printk("UBNT BD mac %02x%02x%02x%02x%02x%02x kidx %d mrev %d serial %s type %s\n",
		ubnt_bd_g.mac[0],ubnt_bd_g.mac[1],ubnt_bd_g.mac[2],ubnt_bd_g.mac[3],ubnt_bd_g.mac[4],
		ubnt_bd_g.mac[5], ubnt_bd_g.kidx, ubnt_bd_g.mrev, ubnt_bd_g.serial, ubnt_bd_g.type);
	return 0;
}

extern const char __dtb_overlays;
static int mtk_nand_load_dtb_overlay_early(size_t dtb_offset)
{
	...
	if (fdt_check_header(&__dtb_overlays + dtb_offset)) {
		printk ("Dtb_overlay not present\n");
		return 0;
	}

	data = kmemdup(&__dtb_overlays + dtb_offset, 0x10000, GFP_KERNEL);
	of_fdt_unflatten_tree(data, NULL, &overlay);
	if (overlay == NULL) {
		printk("%s: failed to unflatten tree\n", __func__);
		err = -EINVAL;
		goto out_err;
	}
	...
	if (err != 0) {
		printk("%s: Failed to resolve tree (%d)\n", __func__, err);
		goto out_err;
	}
	...
	if (err < 0) {
		printk("%s: Failed to create overlay (err=%d)\n",
				__func__, err);
		goto out_err;
	}
...
static int mtk_nand_load_dtb_overlay(void)
{
	struct device_node	*overlay;
	void *data;
	int err;
