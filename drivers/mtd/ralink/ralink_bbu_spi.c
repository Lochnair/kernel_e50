...
static int bbu_mb_spic_trans(const u8 code, const u32 addr, u8 *buf, const size_t n_tx, const size_t n_rx, int flag)
{
	...
	if(flag ==SPIC_WRITE_BYTES)
		ra_dbg("%s: addr=%d n_tx=%d - ", __FUNCTION__, addr, n_tx);
...
//			if (r == 3)
//				ra_dbg("SPI_REG_DATA(%d)=%x\n",q, ra_inl(SPI_REG_DATA(q)));
		}
	}
	...
	size_t *retlen, const u_char *buf)
{
	...
	ra_dbg("%s: to:%x len:%x ", __func__, to, len);
...
#if !defined (GPIO6_TO_SPI_CS) && !defined (USE_SPI_CS1)
