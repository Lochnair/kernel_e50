...
u16 (*get_switch0_pvid)(void) = NULL;
	...
	u32 tx_pkt, u32 tx_byte, u32 tx_err, u32 tx_drop)
{
	...
	pAd->stat.rx_packets += rx_pkt;
	pAd->stat.rx_bytes += rx_byte;
	pAd->stat.rx_errors += rx_err;
	pAd->stat.rx_dropped += rx_drop;
	pAd->stat.tx_packets += tx_pkt;
	pAd->stat.tx_bytes += tx_byte;
	pAd->stat.tx_errors += tx_err;
	pAd->stat.tx_dropped += tx_drop;
	dev_put(dev);
out:
	return 0;
}

int upd_switch0_stats(u32 rx_pkt, u32 rx_byte, u32 rx_err, u32 rx_drop,
	u32 tx_pkt, u32 tx_byte, u32 tx_err, u32 tx_drop)
{
	struct net_device *dev;
	END_DEVICE *ei_local;
	...
	ei_local->stat.rx_packets += rx_pkt;
	ei_local->stat.rx_bytes += rx_byte;
	ei_local->stat.rx_errors += rx_err;
	ei_local->stat.rx_dropped += rx_drop;
	ei_local->stat.tx_packets += tx_pkt;
	ei_local->stat.tx_bytes += tx_byte;
	ei_local->stat.tx_errors += tx_err;
	ei_local->stat.tx_dropped += tx_drop;
	dev_put(dev);
out:
	return 0;
}
...
EXPORT_SYMBOL(upd_switch0_stats);
#endif
...
extern struct ethtool_ops	ra_ethtool_pseudo_ops;
...
#if defined (CONFIG_PSEUDO_SUPPORT) || defined (CONFIG_RAETH_SPECIAL_TAG)
	PSEUDO_ADAPTER *pAd;
#endif
...
#endif
		{
#if 0 // already update these previously
			ei_local->stat.rx_packets++;
			ei_local->stat.rx_bytes += length;
#endif
		}
...
struct rtnl_link_stats64* ra_get_stats64(struct net_device *dev,
						 struct rtnl_link_stats64 *storage)
{
	END_DEVICE *ei_local = netdev_priv(dev);
	memcpy(storage, &ei_local->stat, sizeof(*storage));
...
void gsw_delay_setting(void) 
{
	...
	if( reg_int_val & BIT(25))
	{
		if(sysRegRead(RALINK_ETH_SW_BASE+0x0208) & 0x1) // link up
		{
			printk("GSW Port5 is enable (Force 1G F/D FC=ON)\n");
			*(unsigned long *)(RALINK_ETH_SW_BASE+0x3500) = 0x7e33b;
			mii_mgr_write(31, 0x3500, 0x7e33b);
		} 
		else 
		{
			printk("GSW Port5 is disabled\n");
...
#endif
#if defined (CONFIG_RAETH_SPECIAL_TAG)
	    netif_carrier_on(pdev_raether[port_no]);
#endif
    } else {
	printk("ESW: Link Status Changed - Port%d Link Down\n", port_no);
#if defined (CONFIG_RAETH_SPECIAL_TAG)
	netif_carrier_off(pdev_raether[port_no]);
#endif
    }
}
#endif
...
#if defined (CONFIG_RAETH_SPECIAL_TAG)
static int check_headroom_size(struct sk_buff **skb)
{
	struct sk_buff *new_skb;

	if (skb_headroom(*skb) < ETH_HLEN) {
		new_skb = skb_realloc_headroom(*skb, skb_headroom(*skb)+ETH_HLEN);

		if (new_skb==NULL) {
			printk("skb is not available\n");
			dev_kfree_skb(*skb);
			return 0;
		}
		if ((*skb)->sk)
...
static int vlan2_start_xmit(struct sk_buff* skb, struct net_device *dev)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
...
static int vlan3_start_xmit(struct sk_buff* skb, struct net_device *dev)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
...
static int vlan4_start_xmit(struct sk_buff* skb, struct net_device *dev)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
...
static int vlan5_start_xmit(struct sk_buff* skb, struct net_device *dev)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
...
#if defined(CONFIG_RAETH_STATS64)
        .ndo_get_stats64        = ra_get_stats64,
#else
...
#else
	memcpy(addr.sa_data, ubnt_bd_g.mac, 6);
#endif
...
#if defined(CONFIG_RAETH_STATS64)
struct rtnl_link_stats64* VirtualIF_get_stats64(struct net_device *dev,
						 struct rtnl_link_stats64 *storage)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
	memcpy(storage, &pAd->stat, sizeof(*storage));
	return &pAd->stat;
}
#else
...
int esw_link_status(struct net_device *dev)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
	u32 regValue;

	mii_mgr_read(pAd->mii_info.phy_id, 0x1, &regValue);
	if (regValue & (1 << 2))
		return 1; // link-up
	else
		return 0; // link-down
...
//#endif
    ...
    if (esw_link_status(dev) == 1) // link-up
...
static const struct net_device_ops Vlan1_netdev_ops = {
        .ndo_open               = VirtualIF_open,
        .ndo_stop               = VirtualIF_close,
        .ndo_start_xmit         = vlan1_start_xmit, 
#if defined(CONFIG_RAETH_STATS64)
        .ndo_get_stats64        = VirtualIF_get_stats64,
#else
        .ndo_get_stats          = VirtualIF_get_stats,
...
static const struct net_device_ops Vlan2_netdev_ops = {
        .ndo_open               = VirtualIF_open,
        .ndo_stop               = VirtualIF_close,
        .ndo_start_xmit         = vlan2_start_xmit, 
#if defined(CONFIG_RAETH_STATS64)
        .ndo_get_stats64        = VirtualIF_get_stats64,
#else
        .ndo_get_stats          = VirtualIF_get_stats,
...
static const struct net_device_ops Vlan3_netdev_ops = {
        .ndo_open               = VirtualIF_open,
        .ndo_stop               = VirtualIF_close,
        .ndo_start_xmit         = vlan3_start_xmit, 
#if defined(CONFIG_RAETH_STATS64)
        .ndo_get_stats64        = VirtualIF_get_stats64,
#else
        .ndo_get_stats          = VirtualIF_get_stats,
...
static const struct net_device_ops Vlan5_netdev_ops = {
        .ndo_open               = VirtualIF_open,
        .ndo_stop               = VirtualIF_close,
...
static inline void inc_mac_addr(u8 *mac, int num)
{
	u32 *mac_ptr, mac_addr;
	mac_ptr = (u32 *)&mac[2];
	mac_addr = htonl(*mac_ptr);
	mac_addr += num;
	*mac_ptr = ntohl(mac_addr);
	return;
}
...
#define RT2860_NVRAM	0
	...
	*((volatile u32 *)(ESW_P7_IntMn)) &= ~(MSK_CNT_INT_ALL);
...
#if defined (CONFIG_RAETH_SPECIAL_TAG)
	for(i=0;i<5;i++) {
		pdev_raether[i] = alloc_etherdev(sizeof(PSEUDO_ADAPTER));
		ether_setup(pdev_raether[i]);
		pdev_raether[i]->features = dev->features;
		memcpy(pdev_raether[i]->dev_addr, dev->dev_addr, dev->addr_len);
	}

	strcpy(pdev_raether[0]->name, "eth0");
	strcpy(pdev_raether[1]->name, "eth1");
	strcpy(pdev_raether[2]->name, "eth2");
	...
	inc_mac_addr(pdev_raether[2]->dev_addr, 2);
	...
	for(i=0;i<5;i++) {
		...
		ethtool_init_pseudo(pdev_raether[i], i);
#endif
		register_netdevice(pdev_raether[i]);
		VirtualIF_open(pdev_raether[i]);
	}
#endif

	if (strcmp(ubnt_bd_g.type, "e51") == 0
	    || strcmp(ubnt_bd_g.type, "e52") == 0
	    || strcmp(ubnt_bd_g.type, "e53") == 0) {
...
#if defined (CONFIG_RAETH_SPECIAL_TAG)
	for(i=0;i<5;i++) {
		VirtualIF_close(pdev_raether[i]);
	}
#endif
...
void setup_internal_gsw(void)
{
	...
	*(unsigned long *)(0xbe000060) |= (1 << 15); //set RGMII2 to GPIO mode
...
#if defined (CONFIG_RAETH_SPECIAL_TAG)
	for(i=0;i<5; i++) {	
		unregister_netdev(pdev_raether[i]);
		free_netdev(pdev_raether[i]);
	}
#endif
