...
static u32 pseudo_get_link(struct net_device *dev)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
        PSEUDO_ADAPTER *pAd = netdev_priv(dev);
#else	
	PSEUDO_ADAPTER *pAd = dev->priv;
#endif	
	return mii_link_ok(&pAd->mii_info);
}
...
static int pseudo_set_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
	int rc;
	rc = mii_ethtool_sset(&pAd->mii_info, cmd);
	return rc;
}
...
static int pseudo_get_settings(struct net_device *dev, struct ethtool_cmd *cmd)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
        PSEUDO_ADAPTER *pAd = netdev_priv(dev);
#else
        PSEUDO_ADAPTER *pAd = dev->priv;
#endif
	mii_ethtool_gset(&pAd->mii_info, cmd);
	return 0;
}
 ...
 *	mii_mgr_read wrapper for mii.o ethtool
 */
int mdio_pseudo_read(struct net_device *dev, int phy_id, int location)
{
	unsigned int result;
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
        PSEUDO_ADAPTER *pAd = netdev_priv(dev);
        ...
        PSEUDO_ADAPTER *pAd = dev->priv;
#endif
	mii_mgr_read( (unsigned int) pAd->mii_info.phy_id, (unsigned int)location, &result);
	//printk("\n%s mii.o query= phy_id:%d, address:%d retval:%x\n", dev->name, phy_id, location, result);
	return (int)result;
}
 ...
 *	mii_mgr_write wrapper for mii.o ethtool
 */
void mdio_pseudo_write(struct net_device *dev, int phy_id, int location, int value)
{
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);
#else
...
#endif
	//printk("mii.o write= phy_id:%d, address:%d value:%x\n", phy_id, location, value);
	mii_mgr_write( (unsigned int) pAd->mii_info.phy_id, (unsigned int)location, (unsigned int)value);
	return;
}

struct ethtool_ops ra_ethtool_pseudo_ops = {
	.get_settings		= pseudo_get_settings,
	.get_link               = pseudo_get_link,
	.set_settings		= pseudo_set_settings,
};
