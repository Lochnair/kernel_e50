...
/**
 * ethtool_init_pseudo - Called by raeth_probe to inititialize pseudo network interfaces
 * @dev: device pointer
 * @phyid: PHY ID
 *
 * ethdev_init initilize dev->priv and set to PSEUDO_ADAPTER structure
 *
 */
void ethtool_init_pseudo(struct net_device *dev, unsigned char phyid)
{
#if defined (CONFIG_ETHTOOL) /*&& defined (CONFIG_RAETH_ROUTER)*/
	PSEUDO_ADAPTER *pAd = netdev_priv(dev);

	// init mii structure
	pAd->mii_info.dev = dev;
	pAd->mii_info.mdio_read = mdio_pseudo_read;
	pAd->mii_info.mdio_write = mdio_pseudo_write;
	pAd->mii_info.phy_id_mask = 0x1f;
	pAd->mii_info.reg_num_mask = 0x1f;
	pAd->mii_info.supports_gmii = mii_check_gmii_support(&pAd->mii_info);
	// TODO:   phy_id: 0~4
	pAd->mii_info.phy_id = phyid;
#endif
	return;
}
